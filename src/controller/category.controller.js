...
class CategoryController {
  ...
  async save(req, res) {
    ...
    if (doesCategoryExist) {
      return res.status(400).json({ message: 'Categoria ja existe.' });
    }
  ...
  async update(req, res) {
    ...
    if (req.body.title && req.body.title !== category.title) {
      ...
      if (checkIfTitleExists) {
        return res
          .status(400)
          .json({ message: 'Uma categoria já existe com esse titulo.' });
      }
    }
  ...
  async delete(req, res) {
    ...
    if (!category) {
      return res
        .status(401)
        .json({ message: 'Essa categoria não pode ser encontrada.' });
    }

...
import searchManifestationService from '../../src/app/services/ManifestationService';
import Category from '../../src/app/models/Category';
...
describe('Search Manifestation Service', () => {
  ...
  it('should search for manifestations', async () => {
    ...
    const manifestations = await searchManifestationService.search('rua');
    expect(manifestations).toBeDefined();
    expect(manifestations[0]).toBeDefined();
    expect(manifestations[0]).toEqual(expect.objectContaining({
      title: 'rua',
      description: 'descrição',
      latitude: null,
      type_id: type.id
    }));
    expect(manifestations[0].categories[0]).toEqual(expect.objectContaining({
      title: 'Saneamento',
      id: category.id
    }));
  });
